...
sha256sums=('cfab7e9ad760a046a4182e80b08a04fcb7d95a6e88958b31dabed06121f833be')
...
build() {
  ...
  pip install pyinstaller==6.10.0 PySide6==6.10.0 pyqtgraph==0.13.7 numpy==2.2.4 STO-OSCR==2025.8.10.0 OSCR-django-client==2025.7.19.0 pydantic==2.12.3
...
package() {
  ...
  cp -r "${srcdir}/${srcdir_name}/dist/OSCR-UI" "${pkgdir}/opt/oscr-ui/"

  cat > "${pkgdir}/usr/bin/oscr-ui" <<'EOF'
#!/bin/sh
exec /opt/oscr-ui/OSCR-UI/OSCR-UI "$@"
EOF
...
[Desktop Entry]
Name=OSCR-UI
Comment=OSCR GUI client
Exec=oscr-ui
Icon=/opt/oscr-ui/assets/oscr_icon_small.png
Terminal=false
Type=Application
...
EOF
  ...
  if [ -f "${pkgdir}/opt/oscr-ui/assets/oscr_icon_small.png" ]; then
    install -Dm644 "${pkgdir}/opt/oscr-ui/assets/oscr_icon_small.png" "${pkgdir}/usr/share/icons/hicolor/256x256/apps/oscr-ui.png"
  fi
}
