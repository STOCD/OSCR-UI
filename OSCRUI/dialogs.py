...
class DetectionInfoDialog(QDialog):
    ...
    def build_dialog(self):
        ...
        self._info_frame = create_frame(self._theme)
        self._info_frame.setLayout(QVBoxLayout())
        content_layout.addWidget(self._info_frame)
        seperator = create_frame(self._theme, style='light_frame', size_policy=SMINMAX)
        seperator.setFixedHeight(1)
        content_layout.addWidget(seperator)
        ok_button = create_button(self._theme, tr('OK'))
    ...
    def show_dialog(self, detection_info: Iterable[DetectionInfo]):
        ...
        for detection_step in detection_info:
            ...
            info_label = create_label(self._theme, t)
...
class UploadresultDialog(QDialog):
    ...
    def build_dialog(self):
        ...
        self._title_label = create_label(self._theme, '', 'label_heading', style_override=margin)
        content_layout.addWidget(self._title_label, 0, 0, alignment=ALEFT)
        self._view_button = create_button(self._theme, 'View Online', style_override=margin)
        ...
        self._result_frame = create_frame(self._theme)
    ...
    def show_dialog(self, result: CombatLogUploadV2Response):
        ...
        if result.results:
            for row, line in enumerate(result.results, 1):
                ...
                status_label = create_label(self._theme, '', style_override=icon_table_style)
                status_label.setPixmap(icon)
                status_label.setSizePolicy(SMINMIN)
                result_layout.addWidget(status_label, row, 0)
                name_label = create_label(self._theme, line.name, style_override=table_style)
                ...
                detail_label = create_label(self._theme, line.detail, style_override=table_style)
...
class DialogsWrapper(QObject):
    ...
    def build_message_dialog(self):
        ...
        main_layout = QVBoxLayout()
        main_layout.setContentsMargins(thick, thick, thick, thick)
        dialog_frame = create_frame(self._theme, size_policy=SMINMIN)
        ...
        content_frame = create_frame(self._theme, size_policy=SMINMIN)
        content_layout = QVBoxLayout()
        ...
        self._icon_label_m = create_label(self._theme, '')
        top_layout.addWidget(self._icon_label_m, alignment=ALEFT | AVCENTER)
        self._message_label_m = create_label(self._theme, '')
    ...
    def build_confirmation_dialog(self):
        ...
        cancel_button = create_button(self._theme, tr('Cancel'))
